...
set -e
 ...
 if [[ ( "$#" -gt 2 ) && ( "$#" -lt 5 ) ]]; then
    ...
    echo ""
    echo "Description:"
    echo "  This scripts takes a path to regular changelog and debian control files and"
    ...
    echo ""
    echo "  output path"
    echo "       - Output path for debian changelog. Will overwrite existsing file!"
    echo ""
    ...
    echo "          a list of relese versions and distribution value pairs must be specified."
    echo "          When distribution value changes after a version X, it must only specified at"
    echo "          once as next releases us the previous distribution value. Note that distribution"
    ...
    echo ""
    echo "  Rebuild changelog so that some of the first releases are not included (as the releles" 
